...
function fcpoResetErrorContainers() {
    ...
    if (document.getElementById('fcpo_ccv2_error')) {
        document.getElementById('fcpo_ccv2_error').style.display = 'none';
    }
 ...
 * @param void
 */
jQuery('#fcpo_klarna_combined_agreed, #klarna_payment_selector').change(
    function() {
 ...
 * @param void
 */
var fcpoPayolutionInstallmentCheckAvailability = jQuery('[id="payolution_installment_check_availability"]');
    ...
    fcpoPayolutionInstallmentCheckAvailability[0].addEventListener('click',
        function () {
            ...
            if (payolutionInstallmentCalculationSelection.length > 0) {
                payolutionInstallmentCalculationSelection[0].innerHTML = '<div id="payolution_center_animation"><img src="out/modules/fcpayone/img/ajax-loader.gif"</div>';
            }
...
function fcpoValidateCardTypeCCHosted(e) {
    ...
    var cardType = jQuery('#cardtype option:selected').attr('data-cardtype');
    var oForm = fcpoGetPaymentForm();

    if (paymentId == 'fcpocreditcard' && oForm.fcpo_cc_type.value == 'hosted' && cardType == 'none') {
        jQuery('[id="errorCardType"]').show();
...
function fcpoValidateInputCCHosted(e) {
    ...
    if (paymentId == 'fcpocreditcard' && oForm.fcpo_cc_type.value == 'hosted' && cardType != 'none') {
        ...
        if ($validateResult == 0) {
            e.preventDefault();
            jQuery('[id="errorIncomplete"]').show();
        } else if ($validateResult == 2) {
            jQuery('[id="errorCVC"]').show();
            e.preventDefault();
        } else {
...
function fcpoPaymentFormSubmit (e, sSelectedPaymentMethod, oPaymentForm) {
    ...
    if (sSelectedPaymentMethod != 'fcpocreditcard'
        && sSelectedPaymentMethod != 'fcpocreditcardv2'
...
$(document).ready(function () {
    ...
    const fcpoPaymentNextButtons = $('.sticky-md-top button');
