    ...
    <tr>
        ...
        <td class="edittext">
            <input type="radio" name="editval[oxpayments__fcpoauthmode]" value="preauthorization"
                   [{if $edit->oxpayments__fcpoauthmode->value == 'preauthorization'}]checked[{/if}]>
            [{oxmultilang ident="FCPO_PREAUTHORIZATION"}] [{oxinputhelp ident="FCPO_PREAUTHORIZATION_HELP"}]<br>
            <input type="radio" name="editval[oxpayments__fcpoauthmode]" value="authorization"
                   [{if $edit->oxpayments__fcpoauthmode->value == 'authorization'}]checked[{/if}]>
            [{oxmultilang ident="FCPO_AUTHORIZATION"}] [{oxinputhelp ident="FCPO_AUTHORIZATION_HELP"}]
        </td>
        [{/if}]
    </tr>
    <tr>
                ...
                <tr>
                    <td>
                        <input type="radio" name="editval[oxpayments__fcpolivemode]" value="1"
                               [{if $edit->oxpayments__fcpolivemode->value == '1'}]checked[{/if}]>
                        <strong>[{oxmultilang ident="FCPO_LIVE_MODE"}]</strong><br>
                        <input type="radio" name="editval[oxpayments__fcpolivemode]" value="0"
                               [{if $edit->oxpayments__fcpolivemode->value == '0'}]checked[{/if}]>
                        [{oxmultilang ident="FCPO_TEST_MODE"}]<br>
                    </td>
                    <td>
    ...
    <tr>
        ...
        <td>
            ...
            <input type="checkbox" name="confbools[blFCPOSofoShowIban]" value="1"
                   [{if ($confbools.blFCPOSofoShowIban)}]checked[{/if}]>
        </td>
    </tr>
    [{/if}]
    ...
    <tr>
        ...
        <td>
            ...
            <input type="checkbox" name="confbools[blFCPOTrustlyShowIban]" value="1"
                   [{if ($confbools.blFCPOTrustlyShowIban)}]checked[{/if}]>
        </td>
    </tr>
    [{/if}]
    [{else}]
