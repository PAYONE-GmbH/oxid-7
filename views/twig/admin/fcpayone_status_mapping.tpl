[{include file="headitem.html.twig" title="fcpo_admin_config_status_mapping"|oxmultilangassign}]
        ...
        <tr>
            <td>
                <select name="editval[[{$oMapping->sOxid}]][sPaymentType]" style="width:200px;">
                    ...
                    <option value="[{$oPayment->sId}]" [{if $oMapping->sPaymentType == $oPayment->sId}]selected[{/if}]>
                        [{$oPayment->sTitle}]
                    </option>
                    [{/foreach}]
                </select>
            ...
            <td>
                <select name="editval[[{$oMapping->sOxid}]][sPayoneStatus]" style="width:325px;">
                    ...
                    <option value="[{$oStatus->sId}]" [{if $oMapping->sPayoneStatusId == $oStatus->sId}]selected[{/if}]>
                        [{$oStatus->sTitle}]
                    </option>
                    [{/foreach}]
                </select>
            ...
            <td>
                <select name="editval[[{$oMapping->sOxid}]][sShopStatus]" style="width:180px;">
                    ...
                    <option value="[{$sStatusId}]" [{if $oMapping->sShopStatusId == $sStatusId}]selected[{/if}]>[{
                        oxmultilang ident=$sStatusId noerror=true }]
                    </option>
                    [{/foreach}]
                </select>
            </td>
            <td>
                <input value="X [{ oxmultilang ident=" fcpo_admin_config_delete" }]" name="editval[[{$oMapping->sOxid}
                ]][delete]" onclick="if(!confirm('[{ oxmultilang ident="fcpo_admin_config_delete_confirm"
