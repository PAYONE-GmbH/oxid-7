        ...
        <tr>
            <td>
                <select name="editval[[{$oMapping->sOxid}]][sErrorCode]" style="width:200px;">
                    ...
                    <option value="[{$oError->sErrorCode}]"
                            [{if $oMapping->sErrorCode == $oError->sErrorCode}]selected[{/if}]>[{$oError->sErrorMessage}
                        ] ([{$oError->sErrorCode}])
                    </option>
                    [{/foreach}]
                </select>
            ...
            <td>
                <select name="editval[[{$oMapping->sOxid}]][sLangId]" style="width:180px;">
                    ...
                    <option value="[{$oLang->id}]" [{if $oLang->id == $oMapping->sLangId}]selected[{/if}]>
                        [{$oLang->name}]
                    </option>
                    [{/foreach}]
                </select>
            ...
            <td>
                <input value="X [{oxmultilang ident="fcpo_admin_config_delete"}]"
                       name="editval[[{$oMapping->sOxid}]][delete]"
                       onclick="if(!confirm('[{oxmultilang ident="fcpo_admin_config_delete_confirm"}]')) {return false;}"
                       type="submit">
            </td>
        </tr>
        [{/foreach}]
        ...
        <tr>
            <td>
                <select name="editval2[[{$oMapping->sOxid}]][sErrorCode]" style="width:200px;">
                    ...
                    <option value="[{$oError->sErrorCode}]"
                            [{if $oMapping->sErrorCode == $oError->sErrorCode}]selected[{/if}]>[{$oError->sErrorCode}]
                    </option>
                    [{/foreach}]
                </select>
            </td>
