[{include file="headitem.html.twig" title="fcpo_admin_config_status_forwarding"|oxmultilangassign}]
        ...
        <tr>
            <td>
                <select name="editval[[{$oForwarding->sOxid}]][sPayoneStatus]" style="width:325px;">
                    ...
                    <option value="[{$oStatus->sId}]"
                            [{if $oForwarding->sPayoneStatusId == $oStatus->sId}]selected[{/if}]>[{$oStatus->sTitle}]
                    </option>
                    [{/foreach}]
                </select>
            </td>
            ...
            <td>
                <input value="X [{oxmultilang ident="fcpo_admin_config_delete"}]"
                       name="editval[[{$oForwarding->sOxid}]][delete]" onclick="if(!confirm('[{ oxmultilang ident="
                       fcpo_admin_config_delete_confirm" }]')) {return false;}" type="submit">
            </td>
        </tr>
        [{/foreach}]
    </table>
