<dl>
    ...
    <dt>
        <input id="payment_klarna_combined" type="radio" name="paymentid" value="[{$sPaymentID}]"
               [{if $oView->fcpoIsKlarnaCombined($checkedPaymentId)}]checked[{/if}]>
    ...
    <dd class="[{if  $oView->fcpoIsKlarnaCombined($checkedPaymentId)}]activePayment[{/if}]">
        ...
        <div class="form-group">
            ...
            <div class="col-lg-9">
                <input class="form-control" autocomplete="off" type="text" size="20" maxlength="64"
                       name="dynvalue[fcpo_klarna_telephone]">
            </div>
        </div>
        [{/if}]
        ...
        <div class="form-group">
            ...
            <div class="col-lg-9">
                <input class="form-control" autocomplete="off" type="text" size="20" maxlength="64"
                       name="dynvalue[fcpo_klarna_personalid]">
            </div>
        </div>
        [{/if}]
        ...
        <select id="klarna_payment_selector" class="form-control">
            ...
            <option value="fcpoklarna_installments" [{if $sPaymentID == 'fcpoklarna_installments'}]selected[{/if}]>
                Klarna Slice it
            </option>
            [{/if}]
            [{if $oView->fcpoPaymentActive('fcpoklarna_directdebit')}]
            <option value="fcpoklarna_directdebit" [{if $sPaymentID == 'fcpoklarna_directdebit'}]selected[{/if}]>Klarna
                Pay now
            </option>
            [{/if}]
            [{if $oView->fcpoPaymentActive('fcpoklarna_invoice')}]
            <option value="fcpoklarna_invoice" [{if $sPaymentID == 'fcpoklarna_invoice'}]selected[{/if}]>Klarna Pay
                later
            </option>
            [{/if}]
        </select>
        ...
        <input id="fcpo_klarna_combined_agreed" type="checkbox" name="dynvalue[fcpo_klarna_combined_agreed]"
               value="agreed">
